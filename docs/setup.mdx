---
title: Setup
description: How to setup yeebois-monorepo
---
...
yeebois-monorepo is designed to work on macOS, Linux and Windows. While yeebois-monorepo itself is platform-agnostic, the tooling and dependencies we use have different requirements. We've tested and confirmed that yeebois-monorepo works on the following combinations of operating systems and versions:

| Operating system | yeebois-monorepo version | Node.js version | Notes |
| ---------------- | ----------------- | --------------- | ----- |
...
We're aware of issues on [non-Ubuntu Linux distributions](https://github.com/haydenbleasel/yeebois-monorepo/issues/238). While we don't officially support them, we'd love to know if you get it working!

### pnpm

yeebois-monorepo defaults to using [pnpm](https://pnpm.io/) as a package manager, but you can use [npm](https://www.npmjs.com/), [yarn](https://yarnpkg.com/) or [bun](https://bun.sh/) instead.
...
For the yeebois-monorepo docs (what you're reading now), run:
...
yeebois-monorepo relies on various SaaS products. You will need to create accounts with the following services then set the API keys in your [environment variables](/env):
...
Clone the repo using `yeebois-monorepo` init, replacing `[my-project]` with your project name.

```sh Terminal
npx yeebois-monorepo@latest init [my-project]
```
