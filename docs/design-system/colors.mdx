...
yeebois-monorepo makes use of the CSS variables offered by [shadcn/ui](https://ui.shadcn.com/). They're a brilliant way of abstracting the scaling and maintenance difficulties associated with [Dark Mode](/design-system/dark-mode) and whitelabelling.
...
You can also access colors in JavaScript through the `tailwind` utility exported from `@dank/tailwind-config`, like so:

```tsx component.tsx
import { tailwind } from '@dank/tailwind-config';
