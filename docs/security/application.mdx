...
Arcjet is configured in yeebois-monorepo with two main features: bot detection and the Arcjet Shield WAF:
...
Both the `web` and `app` apps have Arcjet configured with a central client at `@dank/security`  that includes the Shield WAF rules. Each app then extends this client with additional rules:
...
yeebois-monorepo includes a boilerplate setup for Arcjet that protects against common threats to SaaS applications, but since the rules are defined in code, you can easily adjust them dynamically at runtime.
