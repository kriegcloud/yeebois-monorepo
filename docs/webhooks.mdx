...
yeebois-monorepo supports sending webhooks to your users using [Svix](https://www.svix.com/). Svix is an enterprise-ready webhooks sending service.

## How it works

yeebois-monorepo uses the Svix API in a stateless manner. The organization ID from the authenticated user is used as the Svix application UID, which is created automatically when the first message is sent.

## Usage

### Send a webhook

To send a webhook, simply use the `send` function from the `@dank/webhooks` package:

```tsx
import { webhooks } from '@dank/webhooks';
...
To get access to the application portal, use the `getAppPortal` function from `@dank/webhooks` and use the returned URL in an `iframe` on your dashboard.
