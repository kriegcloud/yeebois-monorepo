...
Feature flags (also known as feature toggles) allow you to control access to specific features in your application. yeebois-monorepo provides a simple but powerful feature flag system that can be used to:
...
yeebois-monorepo implements feature flags using Vercel's [Flags SDK](https://vercel.com/docs/workflow-collaboration/feature-flags/flags-pattern-nextjs), which is mostly an architectural pattern for working with feature flags. This setup exists in the `@dank/feature-flags` package, so you can import and use it in as many projects as you'd like.
...
To use the flag in your application, simply import it from the `@dank/feature-flags` package and use it like a normal boolean value.

```tsx page.tsx
import { showAnalyticsFeature } from '@dank/feature-flags';
...
![Vercel Toolbar](/images/vercel-toolbar.png)
