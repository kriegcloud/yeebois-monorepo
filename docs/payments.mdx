...
yeebois-monorepo uses [Stripe](https://stripe.com/) by default for payments and billing. Implementing Stripe in your project is straightforward.
...
```ts page.tsx {1,5}
import { stripe } from '@dank/payments';
...
Stripe Radar supports more [advanced anti-fraud features](https://docs.stripe.com/disputes/prevention/advanced-fraud-detection) if you embed the Stripe JS script in every page load. This is not enabled by default in yeebois-monorepo, but you can add it as follows:
...
```tsx {5, 13}
import '@dank/design-system/styles/globals.css';
import { DesignSystemProvider } from '@dank/design-system';
import { fonts } from '@dank/design-system/lib/fonts';
