...
## 1. Swap out the required dependencies in `@dank/database`
...
Delete everything in `@dank/database/index.ts` and replace it with the following:
...
import { page } from '@dank/database/schema';
